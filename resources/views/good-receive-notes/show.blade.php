            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <span
                            class="text-gray-900 dark:text-white">{{ $goodReceiveNote->received_date->format('F d, Y') }}</span>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <span
                            class="text-gray-900 dark:text-white">{{ $goodReceiveNote->invoice_date ? $goodReceiveNote->invoice_date->format('F d, Y') : '-' }}</span>
                    </div>
                    <div>
                    ...
                    @if ($goodReceiveNote->is_credit && $goodReceiveNote->supplierCredit)
                        <div>
                            ...
                            <a href="{{ route('supplier-credits.show', $goodReceiveNote->supplierCredit) }}"
                                ...
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $goodReceiveNote->supplierCredit->status->badgeColor() }}">
...
>>>>>>> Stashed changes
                        ...
                        <div>
                            <span class="font-medium text-gray-700 dark:text-gray-300">Outstanding:</span>
                            <span class="text-gray-900 dark:text-white font-semibold">LKR
                                {{ number_format($goodReceiveNote->supplierCredit->outstanding_amount, 2) }}</span>
                        </div>
                    @endif
                    <div>
                        <span class="font-medium text-gray-700 dark:text-gray-300">Status:</span>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between">
                        <span class="font-medium text-gray-700 dark:text-gray-300">Subtotal Before Discount:</span>
                        <span class="text-gray-900 dark:text-white">LKR
                            {{ number_format($goodReceiveNote->subtotal_before_discount, 2) }}</span>
                    </div>
                    @if ($goodReceiveNote->discount > 0)
                        <div class="flex justify-between bg-green-50 dark:bg-green-900/30 -mx-2 px-2 py-1 rounded">
                            <span class="font-medium text-green-700 dark:text-green-300">Discount:</span>
                            <span class="text-green-700 dark:text-green-300">LKR
                                {{ number_format($goodReceiveNote->discount, 2) }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="font-medium text-gray-700 dark:text-gray-300">Subtotal:</span>
                        <span class="text-gray-900 dark:text-white">LKR
                            {{ number_format($goodReceiveNote->subtotal, 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="font-medium text-gray-700 dark:text-gray-300">Tax:</span>
                        <span class="text-gray-900 dark:text-white">LKR
                            {{ number_format($goodReceiveNote->tax, 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="font-medium text-gray-700 dark:text-gray-300">Shipping:</span>
                        <span class="text-gray-900 dark:text-white">LKR
                            {{ number_format($goodReceiveNote->shipping, 2) }}</span>
                    </div>
                    <div class="flex justify-between pt-3 border-t-2 border-gray-300 dark:border-gray-600">
                        <span class="font-bold text-gray-900 dark:text-white text-lg">Total:</span>
                        <span class="font-bold text-green-600 dark:text-green-400 text-lg">LKR
                            {{ number_format($goodReceiveNote->total, 2) }}</span>
                    </div>
                    <div class="flex justify-between pt-3 border-t border-gray-200 dark:border-gray-600">
                        <span class="font-medium text-gray-700 dark:text-gray-300">Batches Created:</span>
                        <span class="text-gray-900 dark:text-white">{{ $goodReceiveNote->batches->count() }}</span>
                            ...
                            <tbody class="bg-white dark:bg-gray-700 divide-y divide-gray-200 dark:divide-gray-600">
                                @foreach ($batch->stocks as $stock)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 text-sm text-gray-900 dark:text-white">LKR
                                            {{ number_format($stock->cost_price, 2) }}</td>
                                        <td class="px-4 py-2 text-sm text-gray-900 dark:text-white">LKR
                                            {{ number_format($stock->selling_price, 2) }}</td>
                                        <td class="px-4 py-2 text-sm text-gray-900 dark:text-white">{{ $stock->tax }}%
                                        </td>
                                        <td class="px-4 py-2 text-sm text-gray-900 dark:text-white">{{ $stock->quantity }}
                                        </td>
                                        <td class="px-4 py-2 text-sm">
                                            <span
                                                class="{{ $stock->available_quantity > 0 ? 'text-green-600 dark:text-green-400 font-semibold' : 'text-red-600 dark:text-red-400' }}">
                                                {{ $stock->available_quantity }}
                                            </span>
                                        </td>
                                    </tr>
